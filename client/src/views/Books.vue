        ...
        <b-col cols="12" sm="12" md="10">
          <b-row class="mt-3 text-center">
            <b-col cols="12" sm="12" md="4">
              <b-button
                @click="getBooksOrderByAuthorDesc"
              >Ordenar por autor</b-button>
            </b-col>
            <b-col cols="12" sm="12" md="4">
              <b-button
               @click="getBooksOrderByDateDesc"
              >Ordenar por fecha</b-button>
            </b-col>
            <b-col cols="12" sm="12" md="4">
              <b-button
                  @click="getBooksHasCover"
...
export default Vue.extend({
  ...
  methods: {
    ...
    async getBooksOrderByDateDesc() {
      try {
        this.isLoading = true;
        const response = await Axios.get(`http://localhost:8080/api/books/order-by-publish-date/`);
        this.totalRows = response.data.totalElements;
        this.books = response.data.content;
      } catch (error) {
        throw error;
    ...
    async getBooksOrderByAuthorDesc() {
      try {
        this.isLoading = true;
        const response = await Axios.get(`http://localhost:8080/api/books/order-by-author/`);
        this.totalRows = response.data.totalElements;
        this.books = response.data.content;
      } catch (error) {
        throw error;
    ...
    async getBooksHasCover() {
      try {
        this.isLoading = true;
        const response = await Axios.get(`http://localhost:8080/api/books/has-cover/`);
        this.totalRows = response.data.totalElements;
        this.books = response.data.content;
      } catch (error) {
        throw error;
